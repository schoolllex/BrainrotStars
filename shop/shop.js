...
const ui = {
    ...
    buyQty100: document.getElementById("buy-qty-100"),
...
function getBulk100LastUsed() {
    return Number(localStorage.getItem(SHOP_BULK_100_KEY) || 0);
}

function isBulk100Available() {
    if (!SHOP_BULK_100_ENABLED) return false;
    const lastUsed = getBulk100LastUsed();
    return Date.now() - lastUsed >= BULK_100_COOLDOWN_MS;
}

function getBulk100RemainingMs() {
    const remaining = BULK_100_COOLDOWN_MS - (Date.now() - getBulk100LastUsed());
...
function updateBulk100Availability() {
    const available = isBulk100Available();
...
function onSelectQuantity(qty) {
    if (qty === 100 && !isBulk100Available()) return;
    modalState.qty = qty;
    syncQtySelectionUI();
    updateModalActionTexts();
}
...
function canPurchaseCurrentSelection() {
    ...
    if (modalState.qty === 100 && !isBulk100Available()) {
        updateBulk100Availability();
        return false;
    }
    return true;
...
function markBulk100UsageIfNeeded() {
    if (modalState.qty === 100) {
        localStorage.setItem(SHOP_BULK_100_KEY, `${Date.now()}`);
    }
}
